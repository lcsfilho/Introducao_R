...
paises <- utils::read.csv(
  file = 'https://raw.githubusercontent.com/lcsfilho/Introducao_R/master/Curso%20R/bd/life-expectancy-gdp-per-capita.csv',
...
```{r echo=FALSE, eval=TRUE, message=F, warning=F, fig.width=15, fig.height=7.8}
library(plotly)
paises_sel <- paises %>% 
  filter(pais %in% c('Brazil', 'United Satates', 'China', 'India', 'Kuwait'))

P_lot <-  ggplot(paises, aes(expectativa_vida, round(pib_per_capita, 0), fill = continente)) +
  geom_point(aes(size = populacao, frame = ano, ids = pais, colours="black", stroke = 0.2)) +
  ggplot2::labs(
