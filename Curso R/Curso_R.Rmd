...
```{r eval=FALSE, echo=TRUE}
read.table("https://raw.githubusercontent.com/lcsfilho/dados_abertos/main/escolas.txt",
           sep="|", dec=",")
```
---
## Importando dados no R
...
regioes <- utils::read.csv(
  file = 'https://raw.githubusercontent.com/lcsfilho/dados_abertos/main/regioes.csv',
  sep = ';',
  dec = ','
)
head(regioes)
...
paises <- utils::read.csv(
  file = 'https://raw.githubusercontent.com/lcsfilho/dados_abertos/main/life-expectancy-vs-gdp-per-capita.csv',
...
```{r  fig.align='center', fig.height=6.5, message=FALSE, warning=FALSE}
plot(bahia_mun, col="#64a8d9", border="white")
...
dados_mun_ba <- utils::read.csv(
  file = 'https://raw.githubusercontent.com/lcsfilho/dados_abertos/main/obitos_Infantis_nascidos_vivos_BA_mun_2020.csv',
  sep = ';', encoding = "UTF-8"
)
...
names(bahia_mun) #espiando os nomes das variáveis
```

---
## Mapas
A partir de agora, vamos iniciar a construção dos mapas temáticos. Para criar os intervalos de valores, que serão coloridos no mapa de acordo com sua classe, usamos o pacote `classInt`; e, para colorir os mapas, vamos usar o pacote `RColorBrewer`, que é usado, especialmente, em mapas temáticos.

```{r, eval=-1, warning=FALSE, message=FALSE}
install.packages("classInt", "RColorBrewer")
library(classInt)
library(RColorBrewer)
```

Escolhendo a variável, criando os intervalos e colorindo o mapa
```{r warning=FALSE, message=FALSE}
txmortinf = bahia_mun@data[,"tx_mort_inf"] #Selecionando a variável para o mapa temático
##Intervalos de classe para a variável
intervalos = classIntervals(txmortinf, n = 5, style = "fisher")$brk 
#Escolhendo as cores (repare que a quantidade de intervalos deve ser informada novamente)
cores = brewer.pal(5, "Blues")
```
---
## Mapas

O pacote `RColorBrewer` possui as seguintes paleta de cores:
```{r echo=FALSE, fig.align='center', fig.height=7.5, message=FALSE, warning=FALSE}
display.brewer.all(n=NULL,type='all',colorblindFriendly=TRUE)
...
#Plotando o mapa
par(mar = c(0, 0, 0, 0)) #removendo as margens para dar mais espaço ao mapa
plot(bahia_mun, col=cores[findInterval(txmortinf, intervalos, all.inside = TRUE)],
     axes = TRUE, border = rgb(0.7, 0.7, 0.7), cex = 2)
```


---
## Mapas (alguns argumentos gráficos) 
...
- tmag: tamanho do título do gráfico (rel. outros eixos)
...
Vamos incluir o título no gráfico. Há duas maneiras: a primeira, usando a função `title()`; e a segunda, usando `text()`, na qual precisamos definir as coordenadas do título. Para isso, isamos a função `locator()`, que, quando acionado, o `R` altera o formato do cursor para uma cruz. Após essa alteração, devemos clicar no ponto onde ficará o centro do texto e acionar a techa `esc` para sair da função e observar as coordenadas impressas no console.

**Obs**: \n (quebra de linha

```{r eval=FALSE}
#Aplicando titulo
#title(main = list("Taxa de mortalidade infantil municipal por mil nascidos vivos. Bahia. 2020"),
#cex.main = 0.8)
#locator() # utilizado para encontrar as coordenadas para inserção do título
#ou 
text(-39.84, -8.49,"Taxa de mortalidade infantil municipal por mil nascidos vivos. Bahia. 2020")
```

Agora vamos plotar o mapa
---
...
plot(bahia_mun, col=cores[findInterval(txmortinf, intervalos, all.inside = TRUE)],
     axes = TRUE, border = rgb(0.7, 0.7, 0.7), cex = 2)
text(-42, -8.4,"Taxa de mortalidade infantil municipal por mil nascidos vivos. Bahia. 2020")
```
---

## Mapas

Para finalizar, precisamos criar a legenda

```{r  eval=FALSE, fig.align='center', fig.height=5, message=FALSE, warning=FALSE}
par(mar = c(0, 0, 0, 0)) #removendo as margens para dar mais espaço ao mapa
plot(bahia_mun, col=cores[findInterval(txmortinf, intervalos, all.inside = TRUE)],
     axes = TRUE, border = rgb(0.7, 0.7, 0.7), cex = 2)
text(-42, -8.4,"Taxa de mortalidade infantil municipal por mil nascidos vivos. Bahia. 2020")
...
#Criando legenda
legend(x=-44.4, y=-16.3, legend=leglabs(intervalos),
       fill=cores, bty="n", cex=0.8)
```
Em `leglabs`, defininos o texto dos intervalos já criados.

Em `fill`, as cores também já definidas anteriormente.

`bty` está relacionado à caixa ao redor da legenda. Assume valores "n" ou "o", que significam "não" (sem a caixa) e "sim" (com a caixa), respectivamente. O default é o valor "o".

---
## Mapas


```{r , echo=FALSE, fig.align='center', fig.height=6.5, message=FALSE, warning=FALSE}
par(mar = c(0, 0, 0, 0)) #removendo as margens para dar mais espaço ao mapa
plot(bahia_mun, col=cores[findInterval(txmortinf, intervalos, all.inside = TRUE)],
     axes = TRUE, border = rgb(0.7, 0.7, 0.7), cex = 2)
text(-42, -8.4,"Taxa de mortalidade infantil municipal por mil nascidos vivos. Bahia. 2020")
...
---
## Mapas
Agora vamos criar um mapa pelo pacote `leaflet`, um dos pacotes do `tidyverse`. O **Leaflet** é uma das bibliotecas `JavaScript` para a criação de mapas interativos. É possível gerar esses mapas de forma direta no `R` para usar em documentos `RMarkdown` e `Shiny`.

Como utilizar?

Basicamente, a criação de um mapa pelo `leaflet` passa pelos seguintes passos:

1. Criação do mapa pela função `leaflet()`
2. Inclusão de camadas com as funções `addTiles()`, `addMarkers()` ou `addPolygons()`.
3. Impresão do mapa.

`r RefManageR::Cite(referencias, "Statplace")`


Nos nossos exemplos, vamos substituir a função `addMarkers()` por `setView()`.

Vejamos um simples exemplo de geração de mapa no próximo slide.


---
## Mapas

```{r fig.align='center', fig.height=4.2, fig.width=15}

library(leaflet)

mapa <- leaflet() %>% #Chama a função leaflet()
  addTiles() %>%  #Adiciona um recorte de mapa do OpenStreetMap
  setView(lat=-12.95323880717755, lng=-38.42876781766252, zoom=18) #Coordenadas e zoom inicial
mapa  #Gera o mapa
...
Utilizando os dados da taxa de mortalidade infantil, vamos construir um mapa interativo pelo `leaflet`. Primeiro, vamos ler o arquivo geoespacial dos municípios baianos no formato `.json`.

```{r eval=T, message=F,warning=F}

#Carregando a shapefile (JSON) por município da Bahia
MUN_BA <- rgdal::readOGR(
  dsn = "https://raw.githubusercontent.com/lcsfilho/geodata-br/master/geojson/geojs-29-mun.json", 
  use_iconv = T,
  encoding = "UTF-8"
)
```
...
Vamos espiar a base pelo `Glimpse`

```{r eval=T, message=F,warning=F}

#Espiando a base
dplyr::glimpse(MUN_BA@data)

```

Podemos notar que o código do município no arquivo `json` está identificado como `id` e na base com as taxas de mortalidade infantil, está escrito `CD_MUN`. Então, devemos alterar nossa base para se igualar ao arquivo `json`, afim de unir essas bases corretamente.
---
## Mapas
...
Primeiro, vamos converter o tipo de variável da coluna código do município

```{r}
dados_mun_ba$CD_MUN <- as.character(dados_mun_ba$CD_MUN) #Convertendo o tipo de variável
class(dados_mun_ba$CD_MUN) #Verificando a classe de variável

```

Agora, vamos renomear a coluna do código do município para igualar ao nome que está na base em `json`.
```{r}
#Renomeando a coluna do código da UF no banco ProjIBGE_2018
dados_mun_ba <- dplyr::rename(dados_mun_ba, id = CD_MUN)
names(dados_mun_ba)
...
Agora, vamos unir as bases

```{r eval=T, message=F, warning=F}

MUN_BA@data <- dplyr::left_join(
  x = MUN_BA@data,
  y = dados_mun_ba, 
  by = 'id'
)
...
```{r}
paleta_Mun <- leaflet::colorBin( 
  "Blues", #A paleta de cores escolhida
  MUN_BA$tx_mort_inf, #A variável escolhida
  5, #Quantidade de intervalos para a paleta de cores
  pretty = FALSE #Usado quando os pontos de corte são previamente definidos
)
...
**Construindo o mapa - Parte 1**
```{r eval=FALSE}
Mapa_Parte1 <- leaflet(
  data = MUN_BA, #O dataframe geospacial
  options = leafletOptions(
    zoomControl = TRUE, #opção de controle de zoom
    minZoom = 1, maxZoom = 20, #Configuração do zoom mínimo e máximo
    dragging = TRUE, #Arrasto habilitado
    doubleClickZoom=FALSE  #Zoom com duplo click desabilitado
  )
) %>%
  addTiles() %>% #Adiciona um recorte de mapa do OpenStreetMap
  #Configura as coordenadas e o zoom inicial
  setView(lat = -13.2, lng = -42, zoom = 6) %>% 
addLegend(
    position = "bottomright", 
    opacity = 1,
    title = "Taxa de mortalidade infantil por 1.000 n.v.", 
    pal = paleta_Mun, 
    values = ~tx_mort_inf,
  )
```
...
**Construindo o mapa - Parte 1 + Parte 2**

```{r eval=FALSE}
Mapa_Parte1 %>% 
  addPolygons( stroke = T, #Desenho dos polígonos (ordas) habilitado
    opacity = 1, color = "black",#Opacidade máxima e cor dos polígonos
    weight = 0.5, smoothFactor = 0.0, #Espessira e simplificação das linhas
    fillOpacity = 1, #Opacidade máxima do preenchimento
    #Conteúdo da etiqueta ao passar o mouse
    label=~paste0('Mun: ', name," | Taxa de mortalidade infantil: ", tx_mort_inf, ' / 1.000 n.v.'), 
    fillColor = ~paleta_Mun(tx_mort_inf), #Cores da paleta criada em função da Taxa
    highlight = highlightOptions( #Opções de destaque ao passar o mouse
      weight = 2, color = "black", #Espessura e cor da linha
      fillOpacity = 1, #Opacidade do preenchimento
      bringToFront = TRUE #Trazer para frente
    ), layerId = ~id ) %>% #Chave que identifica o município
  addLegend( #Adição da legenda
    position = "bottomright", opacity = 1, #Posição da legenda e opacidade da legenda
    title = "Taxa de mortalidade infantil por 1.000 n.v.", 
    pal = paleta_Mun, #Paleta de cores utilizada
    values = ~tx_mort_inf, #variável para a legenda
  )
...
```{r echo=FALSE, fig.align='center', fig.height=6.5, fig.width=15}
leaflet(
  data = MUN_BA, #O dataframe geospacial
  options = leafletOptions(
    zoomControl = TRUE, #opção de controle de zoom
  ...
  addTiles() %>% #Adiciona um recorte de mapa do OpenStreetMap
  #Configura as coordenadas e o zoom inicial
  setView(lat = -13.4, lng = -42, zoom = 6) %>%
  addPolygons(
    stroke = T, #Desenho dos polígonos (ordas) habilitado
    opacity = 1, color = "black",#Opacidade máxima e cor dos polígonos
    weight = 0.5, smoothFactor = 0.0, #Espessira e simplificação das linhas
    fillOpacity = 1, #Opacidade máxima do preenchimento
    #Conteúdo da etiqueta ao passar o mouse
  ...
  addLegend( #Adição da legenda
    ...
    opacity = 1, #Opacidade da legenda (0=transparente; 1=opaca)
    title = "Taxa de mortalidade infantil por 1.000 n.v.", 
    pal = paleta_Mun, #Paleta de cores utilizada
    values = ~tx_mort_inf, #variável para a legenda
  )
...
`RMarkdown` é uma ferramenta poderosa para combinar análises e raltórios no mesmo documento.

Para usar o `RMarkdown`, claro, além do `R` e do `RStudio`, você deve iisntalar o pacote do `RMarkdown`

```{r eval=FALSE}
#Instalação do pacote RMarkdown do CRAN no R
install.packages("rmarkdown")
```

O documento `.Rmd` é o formato original do arquivo `RMarkdown`. Ele contém uma combinação de `YAML` (metadados), texto e blocos de códigos (`chunk`).
.
Os principais documetos de saída são `DOC`, `HTML` e `PDF`. Para a saída em PDF, é necessária a instalação do `LATEX`.

Também é possível gerar arquivos em outros formatos, como apresentações em `Power Point`.
---
# Referências
